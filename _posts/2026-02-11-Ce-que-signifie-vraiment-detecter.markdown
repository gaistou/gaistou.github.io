...
Prenons l’exemple de la sensibilité. Jusqu’ici, nous l’avons définie comme le nombre de tests positifs rapporté à l’ensemble des événements malveillants testés, c’est-à-dire la proportion d’événements malveillants effectivement détectés, ou plus formellement : _VP / (VP + FN)_. Mais on peut aussi dire que c’est la probabilité qu’un événement soit testé positif sachant qu’il est malveillant, que l’on note _P(test positif | malveillant)_. Les probabilités et les ratios sont toujours deux facettes d’une même pièce.
...
Par exemple, si l’on applique cette formule pour calculer la précision, c’est-à-dire P(M | T+), on obtient :
...
- P(M | T+) = (P(T+ | M) × P(M)) / \[P(T+ | M) × P(M) + P(T+ | non M) × P(non M)\]

On peut maintenant observer l’équation et essayer d’interpréter chacun des termes. On voit :
- P(M | T+) : la probabilité qu’un événement soit malveillant sachant qu’il a été testé positif, c’est-à-dire la précision.
- P(T+ | M) : la probabilité qu’un événement soit détecté sachant qu’il est malveillant, c’est-à-dire la sensibilité.
- P(M) : la probabilité qu’un événement pris au hasard soit malveillant, c’est-à-dire la prévalence.
- P(T+ | non M) : la probabilité qu’un événement déclenche une alerte sachant qu’il n’est pas malveillant, c’est-à-dire (1 − spécificité).
...
- précision = (sensibilité × prévalence) / (sensibilité × prévalence + (1 − spécificité) × (1 − prévalence))
...
precision = (0,999 × 0,00001) / (0,999 × 0,00001 + (1 − 0,999) × (1 − 0,00001))  
≈ 1 %
...
Une probabilité mesure la chance qu’un événement se produise, entre 0 et 1. Dans notre cas, par exemple, P(M) vaut 1 / 100 000, car j’ai 99 999 événements non malveillants pour 1 événement malveillant. Cette idée de _99 999 contre 1_ s’appelle une _odds_. On peut l’écrire : odds(M) = 1 / 99 999 et la prononcer _1 contre 99 999_. C’est exactement le même concept que les cotes que l’on retrouve dans les paris sportifs.
...
- = 1 / 99 999 ; on retrouve bien le bon résultat.
...
- précision = P(M | T+) = (sensibilité × prévalence) / (sensibilité × prévalence + (1 − spécificité) × (1 − prévalence))
...
- précision (formulée comme une _odds_) = odds(M | T+) = (sensibilité / (1 − spécificité)) × odds(M)

Autrement dit, les _odds_ qu’un événement soit malveillant sachant qu’il a déclenché une alerte, ce sont les _odds_ de la prévalence multipliées par (sensibilité / (1 − spécificité)). Cette dernière valeur est en fait ce que les mathématiciens appellent le _likelihood ratio_, noté LR+.
...
- Au commencement, on a 100 000 000 d’événements avec une prévalence de la malveillance de 1 / 100 000. Si l’on y pioche un événement au hasard dans nos données, les _odds_ que l’événement soit malveillant sont donc de 1 contre 99 999.
- Puis on passe chaque événement dans un test de détection avec 99,9 % de sensibilité et 99,9 % de spécificité, et on obtient ~100 000 alertes. Si l’on y pioche une alerte au hasard, les _odds_ que l’alerte corresponde à un événement malveillant sont :  
    odds(M | T+) = odds(M) × LR+ = 1 / 99 999 × 998 ≈ 1 / 100.
...
```
On l'a vu, sous forme de probabilité, la précision s'écrit P(M | T+).

On sait que :

odds(M | T+) = P(M | T+) / (1 - P(M | T+))`
             = P(M | T+) / P(non M | T+)
             
D'après le théorème de Bayes :

P(M | T+) = (P(T+ | M) * P(M)) / P(T+) ; et
P(non M | T+) = (P(T+ | non M) * P(non M)) / P(T+)

Donc :

odds(M | T+) = ((P(T+ | M) * P(M)) / P(T+)) / ((P(T+ | non M) * P(non M)) / P(T+))

On simplifie en haut et en bas par P(T+) :

odds(M | T+) = (P(T+ | M) * P(M)) / (P(T+ | non M) * P(non M))

On peut couper la formule en 2 :

odds(M | T+) = (P(T+ | M) / P(T+ | non M)) * (P(M) / P(non M)) 
             = (P(T+ | M) / P(T+ | non M)) * (P(M) / (1- P(M)))

On reconnait tout à droite une expressions de odds :

odds(M | T+) = (P(T+ | M) / P(T+ | non M)) * odds(M)

On sait que :

P(T+ | M) c'est la sensibilité ; et
P(T+ | non M) c'est 1 - specificité
