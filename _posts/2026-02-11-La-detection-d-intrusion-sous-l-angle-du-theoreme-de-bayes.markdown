...
Puis, pour montrer la formule développée jusqu’au bout, on peut décomposer le dénominateur avec la loi des probabilités totales :
...
On peut maintenant reprendre la matrice de confusion de l’article précédent :
...
- Deuxièmement, l'écriture sous forme de probabilités met en évidence le rôle de chaque métrique. La sensibilité et la spécificité sont conditionnées par la réalité (*sachant* M ou *sachant* non M), alors que la précision est conditionnée par le résultat du test (*sachant* T+). Autrement dit, **la sensibilité et la spécificité nous informent sur le test selon la réalité, alors que la précision nous informe sur la réalité selon le test.** 
...
Je ne vais pas entrer dans les détails des raisons pour lesquelles on voudrait parler en probabilités ou en _odds_ en général, mais sachez que ce sont deux représentations mathématiques différentes. Certains raisonnements sont plus intuitifs en probabilités, d’autres le sont davantage en _odds_. Et ça tombe bien, on connaît un très bon exemple de formule intéressante à convertir en _odds_ : le théorème de Bayes.
...
Il ne faut pas voir un système de détection comme un oracle. Fondamentalement, ce n’est qu’un modificateur de probabilités. Il transforme les _odds_ _a priori_ que l’événement soit malveillant en _odds_ _a posteriori_. Le multiplicateur de transformation dépend de la performance du test, c’est-à-dire de sa sensibilité et de sa spécificité, sous une forme précise : le _likelihood ratio_.
...
Mais nous avons maintenant une intuition beaucoup plus saine de ce qu’est réellement la détection, et nous avons de nouveaux outils mathématiques pour développer des solutions.
